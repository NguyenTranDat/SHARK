...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, h, h_xReact, h_oReact, utt_xReact_mask, utt_oReact_mask):
        ...
        a_input_SS = torch.cat(
            [
                Wh.repeat(1, 1, N).view(-1, N * N, self.out_features),
                (Wh + h_xReact).repeat(1, N, 1),
            ],
            dim=-1,
        ).view(-1, N, N, 2 * self.out_features)
        a_input_DS = torch.cat(
            [
                Wh.repeat(1, 1, N).view(-1, N * N, self.out_features),
                (Wh + h_oReact).repeat(1, N, 1),
            ],
            dim=-1,
        ).view(-1, N, N, 2 * self.out_features)
    ...
    def _prepare_attentional_mechanism_input(self, Wh):
        ...
        # Repeat each feature vector N times: e1, e1, ..., e1, e2, e2, ..., e2, ..., eN, eN, ..., eN
        Wh_repeated_in_chunks = Wh.repeat_interleave(N, dim=0)
        # Repeat the feature matrix N times：e1, e2, ..., eN, e1, e2, ..., eN, ..., e1, e2, ..., eN
        Wh_repeated_alternating = Wh.repeat(N, 1)
        # shape == (N * N, out_features)
        all_combinations_matrix = torch.cat(
            [Wh_repeated_in_chunks, Wh_repeated_alternating], dim=1
        )  # Combine each node with all other nodes: (N * N, 2 * out_features)
